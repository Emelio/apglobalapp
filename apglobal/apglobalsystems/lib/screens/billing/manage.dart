...
class ManageCardState extends State<ManageCard> {
  ...
  listofCards() async {
  Map<String, dynamic> cardsList = await Communicator.getCard();

  if(cardsList['cardDetails'].length != 0) {

    List<dynamic> listData = new List();
    listData = cardsList['cardDetails'];

    List<Widget> item = List<Widget>();
    for (var i = 0; i < listData.length; i++) {

      double cardNumberdouble = listData[i]['cardNumber'];
      String cardNumber = cardNumberdouble.toStringAsFixed(0);

      String status = listData[i]['finalStatus'];

      String stage = listData[i]['stage'];
                                  ...
                                  children: <Widget>[
                                    ...
                                    RaisedButton(
                                      ...
                                      onPressed: () async {
                                        ...
                                        // update card
                                        Communicator.verifyCard(cardNumber, double.parse(amount.text), data['id']);
